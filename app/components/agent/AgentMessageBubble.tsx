    ...
    >
      ...
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
          isUser ? 'bg-blue-500' : isSystem ? 'bg-amber-500' : 'bg-gradient-to-br from-purple-500 to-pink-500'
        }`}
      >
        {isUser ? (
      ...
      <div className={`flex-1 max-w-[85%] ${isUser ? 'text-right' : 'text-left'}`}>
        ...
        >
          ...
          {/* Streaming indicator */}
          {isStreaming && <span className="inline-block w-2 h-4 ml-1 bg-purple-500 animate-pulse rounded-sm" />}
        </div>

        {/* Metadata */}
        {message.metadata && !isStreaming && (
          <div className="mt-1 text-xs text-bolt-elements-textTertiary">
            {message.metadata.tokensUsed && <span>{message.metadata.tokensUsed.toLocaleString()} tokens</span>}
          </div>
        )}
      </div>
    </motion.div>
